...
def enter_add_mode():
    """FIX: Explicitly clears previous data so the form starts blank."""
    st.session_state.selected_customer = None
...
# --- 5. TOP SECTION: REFRESHED STATS DASHBOARD ---
if st.session_state.view_mode == "list":
    if not df_all.empty:
        ...
        with m1:
            st.metric("Total Registry", f"{total_cust} Pax")
        with m2:
            st.metric("Compliance Risk", f"{expired_count} Expired", delta=f"{expired_count} Alerts", delta_color="inverse")
        with m3:
            st.metric("Foreign Licenses", f"{intl_count} Intl")
    st.divider()
...
if st.session_state.view_mode in ["add", "edit"]:
    ...
    with st.container(border=True):
        st.subheader("📝 Edit Customer Profile" if cust else "➕ New Customer Registration")
        
        with st.form("customer_form", clear_on_submit=False):
            st.write("### Personal Details")
            # If cust is None (Add Mode), these will correctly default to empty strings
            ...
            f_phone = c2.text_input("Mobile Phone", value=cust['phone'] if cust and not pd.isna(cust['phone']) else "")
            ...
            f_expiry = c4.date_input("License Expiry Date", value=safe_date(cust['dl_expiry'] if cust else None, default=date.today()))
            ...
            if cust and cust.get('license_scan_path'):
                try:
                    ...
                    st.info("✅ Current ID Scan verified on file.")
                    st.link_button("👁️ View Existing ID Scan", img_url['signedURL'])
                except:
                    st.caption("No accessible scan path found.")

            license_file = st.file_uploader("Upload ID Scan (Optional)", type=['png', 'jpg', 'jpeg', 'pdf'])

            sub_col, can_col = st.columns(2)
            if sub_col.form_submit_button("💾 Save to Registry", use_container_width=True):
                age = (date.today() - f_dob).days // 365
                if not f_name or not f_dl:
                    st.error("Mandatory Fields: Full Name and License Number.")
                elif age < 21:
                    st.error(f"Compliance Error: Age is {age} (Policy requires 21+).")
                elif f_expiry < date.today():
                    st.error("Compliance Error: The provided Driver's License has expired.")
                else:
                    try:
                        ...
                        st.error(f"Database/JSON Error: {e}")
...
if st.session_state.view_mode == "list":
    # FIXED: "Add New" now calls the enter_add_mode function to clear the form
    st.button("➕ Add New Customer", on_click=enter_add_mode, use_container_width=True)
    
    search = st.text_input("🔍 Quick Search", placeholder="Search by name or license number...")
    
    if not df_all.empty:
        st.write("---")
        h1, h2, h3, h4 = st.columns([3, 2, 2, 1])
        h1.caption("**NAME / COMPLIANCE**")
        h2.caption("**LICENSE / EXPIRY**")
        h3.caption("**PHONE**")
        h4.caption("**ACTION**")

        for _, row in df_all.iterrows():
            s_name = str(row['name']) if not pd.isna(row['name']) else "Unknown"
